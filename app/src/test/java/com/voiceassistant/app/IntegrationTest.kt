...
class IntegrationTest {
    ...
    fun setup() {
        ...
        every { context.checkSelfPermission(any()) } returns
        ...
        every {
            anyConstructed<OkHttpClient.Builder>()
                    .build()
                    .newWebSocket(any(), capture(slot<WebSocketListener>()))
        } answers
                {
    ...
    fun `test full interruption flow from speech detection to audio stop`() {
        ...
        val responseCreated = JsonObject().apply { addProperty("type", "response.created") }
        ...
        val speechStarted =
                JsonObject().apply { addProperty("type", "input_audio_buffer.speech_started") }
        ...
        val responseCancelled = JsonObject().apply { addProperty("type", "response.cancelled") }
        ...
        verify(atLeast = 1) {
            mockWebSocket.send(match<String> { it.contains("input_audio_buffer.append") })
        }
    }
        ...
        val errors =
                listOf(
                        ...
                        JsonObject().apply {
                            ...
                            add(
                                    "error",
                                    JsonObject().apply { addProperty("code", "response_not_found") }
                            )
                        }
                )

        errors.forEach { error -> webSocketListener.onMessage(mockWebSocket, gson.toJson(error)) }
